...
func activate():
	...
	get_node("Tween").interpolate_property(get_node("Camera2D"), "position", get_node("Camera2D").position, Vector2(-150, 0), 2.0, Tween.TRANS_CUBIC, Tween.EASE_IN);
...
func start_engine():
	is_engine_on = true;
	if (is_interactive):
		get_node("VisibleCar/GunFront").can_shot = true;
		get_node("VisibleCar/GunTop").can_shot = true;
...
func _process(delta):
	...
	if is_interactive and Input.is_action_pressed("ui_up") and not is_jumping and velocity.x >= MIN_SPEED:
